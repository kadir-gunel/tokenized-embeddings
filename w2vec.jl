...
get_negative_samples(contexts, vocab_size, k) = rand(1:vocab_size, k, length(contexts)) # k,b
...
function loss(model::Word2Vec, center::Vector{Int}, context::Vector{Int}, neg_samples::Matrix{Int})
    ...
    dsize, bsize = inputs |> size
    inputs = reshape(inputs, 1, dsize, bsize) # (D, 1, B)

    neg_scores = sum(log.(sigmoid.(-batched_mul(inputs, negouts))))
